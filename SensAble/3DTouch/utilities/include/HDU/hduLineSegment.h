...
public:
    ...
    explicit hduLineSegment(
        const typename hduLine<T>::VecType &p1 = VecType(),  // Line passes through p1
        const typename hduLine<T>::VecType &p2 = VecType()); // Line passes through p2
    ...
    {
        return (m_p1 + (t * m_length) * m_unitDirection);
    }
