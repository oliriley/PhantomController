...
void hluFitWorkspace(const HLdouble projMatrix[16])
{
    ...
    HDdouble uniformScale = (min(scale[0],scale[1])+scale[2])/2.0;
...
void hluFitWorkspaceFront(const HLdouble projMatrix[16])
{
    ...
    HDdouble wsToCan = 1.0 / min(min(wsExtents[0], wsExtents[1]), wsExtents[2]);

    // Fit the box extents to a canonical volume by fitting the max dimension
    HDdouble canToBox = max(max(boxExtents[0], boxExtents[1]), boxExtents[2]);
